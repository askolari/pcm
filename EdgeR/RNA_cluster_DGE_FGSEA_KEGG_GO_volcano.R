...
dge_cluster3 <- dge_filtered[, metadata_combined$Cluster == 3]
...
lrt_cluster3_ure_vs_ure_dmxaa <- glmLRT(fit_cluster3)

# Extract top differentially expressed genes
top_genes_cluster3 <- topTags(lrt_cluster3_ure_vs_ure_dmxaa, n = 100)
print(top_genes_cluster3)

# Save the results
write.csv(lrt_cluster3_ure_vs_ure_dmxaa, "dge_cluster3.csv")
...
# Perform the likelihood ratio test (LRT) for the comparison with the baseline "PBS_RT_DMXAA"
lrt_cluster4_pbs_vs_ure_rt_dmxaa <- glmLRT(fit_cluster4, coef = 3) 

# Extract top differentially expressed genes for "PBS_RT_DMXAA vs Ure_RT_DMXAA"
top_genes_cluster4 <- topTags(lrt_cluster4_pbs_vs_ure_rt_dmxaa, n = Inf)

# Save the results
write.csv(top_genes_cluster4$table, file = "cluster4_DEGs_PBS_RT_DMXAA_vs_Ure_RT_DMXAA.csv")

#Now for second comparison PBS vs Ure_RT

# Perform the likelihood ratio test (LRT) for the comparison "PBS_RT_DMXAA vs Ure_RT"
lrt_cluster4_pbs_vs_ure_rt <- glmLRT(fit_cluster4, coef = 2)  # coef = 2 corresponds to "Ure_RT vs PBS_RT_DMXAA"

# Extract top differentially expressed genes for "PBS_RT_DMXAA vs Ure_RT"
top_genes_cluster4_pbs_vs_ure_rt <- topTags(lrt_cluster4_pbs_vs_ure_rt, n = Inf)

# Print the top differentially expressed genes
print(top_genes_cluster4_pbs_vs_ure_rt)

# Save the results to a CSV file
write.csv(top_genes_cluster4_pbs_vs_ure_rt$table, file = "cluster4_DEGs_PBS_RT_DMXAA_vs_Ure_RT.csv")

###and for ure_rt vs ure_RT_DMXAA

contrast_ure_rt_vs_ure_rt_dmxaa <- makeContrasts(condition_cluster4Ure_RT_DMXAA - condition_cluster4Ure_RT, levels=design_cluster4)

lrt_cluster4_ure_rt_vs_ure_rt_dmxaa <- glmLRT(fit_cluster4, contrast = contrast_ure_rt_vs_ure_rt_dmxaa)

top_genes_cluster4_ure_rt_vs_ure_rt_dmxaa <- topTags(lrt_cluster4_ure_rt_vs_ure_rt_dmxaa, n = Inf)

print(top_genes_cluster4_ure_rt_vs_ure_rt_dmxaa)

# Save the results to a CSV file
write.csv(top_genes_cluster4_ure_rt_vs_ure_rt_dmxaa$table, file = "cluster4_DEGs_Ure_RT_vs_Ure_RT_DMXAA.csv")
...
file_paths <- c(
  "C:/Users/agsko/dev/pcm/EdgeR/dge_cluster3.csv",
  "C:/Users/agsko/dev/pcm/EdgeR/cluster4_DEGs_PBS_RT_DMXAA_vs_Ure_RT.csv",
  "C:/Users/agsko/dev/pcm/EdgeR/cluster4_DEGs_Ure_RT_vs_Ure_RT_DMXAA.csv",
  "C:/Users/agsko/dev/pcm/EdgeR/cluster4_DEGs_PBS_RT_DMXAA_vs_Ure_RT_DMXAA.csv",
  "C:/Users/agsko/dev/pcm/EdgeR/dge_cluster1.csv"
)
...
# Loop through each file and filter upregulated and downregulated genes
for (file in files) {
  # Read the data
  data <- read.csv(file)
  
  # Filter upregulated genes (logFC > 1 and & PValue < 0.05)
  upregulated_genes <- subset(data, logFC > 1 & PValue < 0.05)
  
  # Filter downregulated genes (logFC < -1 and & PValue < 0.05)
  downregulated_genes <- subset(data, logFC < -1 & PValue < 0.05)
  
  # Save upregulated genes to new file
  upregulated_file <- gsub(".csv", "_upregulated.csv", file)
  write.csv(upregulated_genes, upregulated_file, row.names = FALSE)
  
  # Save downregulated genes to new file
  downregulated_file <- gsub(".csv", "_downregulated.csv", file)
  write.csv(downregulated_genes, downregulated_file, row.names = FALSE)
...
dge_file <- read.csv("C:/Users/agsko/dev/pcm/EdgeR/dge_cluster3.csv")
...
ranks <- setNames(dge_file$logFC, dge_file$X)
...
fgsea_res1 <- fgsea_res[fgsea_res$padj < 0.05, ]

# Plot the results
fgsea_plot <- ggplot(fgsea_res1, aes(reorder(pathway, NES), NES)) +
  geom_bar(stat = "identity", aes(fill = padj), show.legend = TRUE) +
  scale_fill_gradient(low = "blue", high = "red") +
  coord_flip() +
  labs(title = "Enriched Pathways (GSEA) for Cluster 3(MH)\n(Ure vs Ure_DMXAA)", 
       x = "Pathway", 
...
ggsave("fgsea_cluster1_mh_pathways.png", fgsea_plot)

#do the same for cluster 1 and 4, and mh to m8
