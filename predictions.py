...
for sample_id in sample_ids:
    ...
    input_file = os.path.join(base_file_path, f'{sample_id}_class1_predictions.csv')
    
    # Load the peptide prediction dataset
    predictions_df = pd.read_csv(input_file)
    
    # Primary filter: IC50 < 500 nM
    high_affinity_peptides = predictions_df[predictions_df['netmhcpan_ba IC50'] < 500]
    
    # Secondary filter: Percentile rank < 1%
    high_affinity_peptides = high_affinity_peptides[high_affinity_peptides['netmhcpan_ba percentile'] < 1]
    
    # Sort by IC50 value and Percentile rank (for better prioritization)
    prioritized_peptides = high_affinity_peptides.sort_values(by=['netmhcpan_ba IC50', 'netmhcpan_ba percentile'])
...
for sample_id in sample_ids:
    ...
    # Filter: IC50 < 50 nM
    high_affinity_peptides = predictions_df[predictions_df['netmhcpan_ba IC50'] < 50]
...
combined_output_file = os.path.join(base_file_path, 'high_peptides_combined_all_samples.csv')
